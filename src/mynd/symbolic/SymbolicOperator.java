...
public class SymbolicOperator extends Operator {
  ...
  /**
   * Effect of this operator. Note: If this operator has no effect, then effect is null.
   */
   ...
   */
  public SymbolicOperator(String name, SymbolicCondition precondition, BDD effect,
  ...
  /**
   * Two BDD operators are equal if their precondition, their effect and their observations are
   * equal.
   */
  @Override
  ...
  /**
   * Abstract this symbolic operator to given pattern. First corresponding explicit op is
   * abstracted, and after that, a symbolic operator is created from abstracted explicit operator.
   *
   * @param pattern
  ...
  public SymbolicOperator abstractToPattern(Set<Integer> pattern) {
    ExplicitOperator abstractedExplicitOp =
        Global.BDDManager.operatorMapping.get(this).abstractToPattern(pattern);
