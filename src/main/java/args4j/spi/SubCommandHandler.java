 ...
 * <tt>svn</tt>, where the first argument to the command designates a sub-command (say <samp>git
 * checkout</samp>), then everything that follows afterward are parsed by this sub-command (which is
 * usually different depending on which sub-command was selected.)
 *
 * <p>
 ...
 *
 * <li>
 * The same field/setter must be also annotated with {@link SubCommands} that specify possible
 * sub-commands.
 *
 * <li>
 * Any {@link Option}s that you define in the <tt>Git</tt> class above can parse options that appear
 * prior to the sub-command name. This is useful for defining global options that work across
 * sub-commands.
 *
 * <li>
 * The matching sub-command implementation gets instantiated with the default constructor, then a
 * new {@link CmdLineParser} will be created to parse its annotations.
 *
 * <li>
 ...
 * sophisticated sub-command lookup, such as through META-INF/services, <a
 * href="http://sezpoz.java.net/">sezpoz</a>, or <a
 * href="https://github.com/jenkinsci/lib-annotation-indexer">annotation indexer</a>.
 *
