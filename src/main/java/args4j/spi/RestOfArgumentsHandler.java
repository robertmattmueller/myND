 ...
 *
 * <p>
 * Used with {@link Argument}, this implements a semantics where non-option token causes the option
 * parsing to terminate. An example of this is <tt>ssh(1)</tt>, where <samp>ssh -p 222 abc</samp>
 * will treat <samp>-p</samp> as an option to <tt>ssh</tt>, but <samp>ssh abc -p 222</samp> is
 * considered to have no option for <tt>ssh</tt>.
 *
 * @author Kohsuke Kawaguchi
 */
public class RestOfArgumentsHandler extends OptionHandler<String> {
  public RestOfArgumentsHandler(CmdLineParser cmdLineParser, OptionDef optionDef,
