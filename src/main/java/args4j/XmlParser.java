  ...
  public void parse(InputSource xml, CmdLineParser parser, Object bean) {
    try {
      ...
      for (ConfigElement ce : config.options) {
        ...
        parser.addOption(
            Setters.create(parser, findMethodOrField(bean, ce.field, ce.method), bean), option);
      }
      for (ConfigElement ce : config.arguments) {
        Argument argument = new ArgumentImpl(ce);
        parser.addArgument(
            Setters.create(parser, findMethodOrField(bean, ce.field, ce.method), bean), argument);
      }
    } catch (Exception e) {
