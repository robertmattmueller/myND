 ...
 *
 * <p>
 * BDDDomains are called "finite domain blocks" in Section 2.9 of the buddy documentation. A
...
public abstract class BDDDomain {
  ...
  public BDD buildEquals(BDDDomain that) {
    if (!size().equals(that.size())) {
      throw new BDDException("Size of " + this + " != size of that " + that + "( " + size()
          + " vs " + that.size() + ")");
    }
   ...
   * [i_1,...,i_k].
   * <p>
   * Be careful when using this method for BDDs with a large number of entries, as it allocates a
