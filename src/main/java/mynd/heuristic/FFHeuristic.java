...
/**
 * Relaxed planning graph heuristic as implemented in the FF planning system by J. Hoffmann.
 *
...
public class FFHeuristic extends Heuristic {
  ...
  /**
   * Mapping from variable indices to mappings from value indices to FF propositions
   */
  ...
  /**
   * FFProposition corresponding to goal. This is either the auxiliary variable corresponding to the
   * conjunction of the goal atoms or the variable corresponding to the root node of BDD.
   */
  ...
  /**
   * The strategy that is used to evaluate the relaxed planning graph. By default FF is used.
   */
   ...
   */
  private void addRule(FFProposition head, Collection<Pair<Integer, Integer>> opBody,
      ExplicitOperator op) {
  ...
  /**
   * Build goal rules, i.e., rules stating when a goal proposition has been reached.
   *
  ...
  private void buildGoalRules(ExplicitCondition goal) {
    ...
    ExplicitOperator goalOperator =
        new ExplicitOperator("goalRule", precondition, null, obs, false, 0);
  ...
  private void buildRules() {
    ...
    for (OperatorRule axiom : problem.axioms) {
      ...
      ExplicitOperator dummy =
          new ExplicitOperator("axiom", null, null, Collections.<Pair<Integer, Integer>>emptySet(),
              false, 0);
  ...
  /**
   * Build the rules corresponding to the given operator. E.g., an operator <tt>(pre,add,del)</tt>
   * with <tt>pre = {1:0, 3:1}</tt>, <tt>add = {2:1, 3:2}</tt>, and <tt>del = {4:0, 3:1}</tt>, which
   * has the relaxed operator <tt>({1:0, 3:1}, {2:1, 3:2}, {})</tt>, yields the two rules
  ...
  /**
   * Collect the rules required to reach a given effect proposition. The collected rules are
   * accumulated in the set <tt>collectedRules</tt>.
   *
  ...
  /**
   * Explore the relaxed planning graph starting with the propositions true in the current state
   * until either the goal is reached or the planning graph has levelled off.
   *
   * @return True iff there is a goal condition for which all propositions have been reached.
   */
  ...
  /**
   * Extract the value of the FF heuristic from the relaxed planning graph by performing a backward
   * sweep from the reached goal proposition. If there was more than one goal condition, the
   * extraction starts with the first goal condition (= conjunction of goal propositions) reached.
   *
   * @return The value of the FF heuristic extracted from the relaxed planning graph
   */
  private int extractFFValue() {
    ...
    if (goalProposition == null) {
      System.err
      .println("Error in FFHeuristic.extractFFValue: none of the goal monomials has been reached yet.");
   ...
   * @return The evaluation of the given state, i.e., the length of a relaxed plan.
   *         <tt>FFHeuristic.INFINITE_HEURISTIC</tt> indicates that the relaxed planning graph
   *         exploration has levelled off without reaching all the goals.
   */
    ...
    switch (Global.options.getHeuristicAggregationMethod()) {
      ...
      case AVERAGE:
        return HeuristicValueAggregation
            .average(((BeliefState) state).getRandomExplicitWorldStates(Global.options
                .getNumberOfWorldStatesToBeSampled()), this);
      case MAX:
        return HeuristicValueAggregation
            .maximize(((BeliefState) state).getRandomExplicitWorldStates(Global.options
                .getNumberOfWorldStatesToBeSampled()), this);
      default:
        assert false;
        return -1;
  ...
  /**
   * Initialize the propositions and mark those as reached which are true in the current state. Put
   * them into the queue of reachable propositions.
   *
  ...
  /**
   * Initialize rules by setting their unsatisfied precondition counts to the respective numbers of
   * preconditions, and trigger rules with empty bodies.
   */
  ...
  /**
   * Trigger a rule and mark its effect/head proposition as reached and add it to the queue of
   * reached propositions, unless it has already been reached before. Set the cost of reaching the
   * proposition.
   *
  ...
  private void triggerRule(FFRule rule) {
    ...
    switch (strategy) {
      ...
      case MAX:
        // effect.reachedBy = rule;
        if (DEBUG) {
          System.out.println("case MAX: trigger rule for effect " + effect + " of op "
              + rule.operator);
        }
        double cost = rule.operator.getCost();
        double maxCost = 0;
        ...
        if ((cost + maxCost) < effect.reachCost
            || effect.reachCost == FFProposition.INFINITE_REACH_COST) {
          effect.reachedBy = rule;
          cost += maxCost;
...
/**
 * A proposition, i.e., a variable-value pair, together with a list of FF rules in whose
 * precondition it occurs, the rule by which it was reached (initially <tt>null</tt>) and to cost to
 * reach it.
 *
 * @author Robert Mattmueller
 */
...
/**
 * An FF rule is a rule (head :- body) where the body is the precondition of an operator and the
 * head is one of the operator's add effects.
 *
 * @author Robert Mattmueller
 */
class FFRule implements Comparable<FFRule> {
  ...
  /**
   * Indicates that the counter of unsatisfied preconditions of a rule has not yet been initialized.
   */
  ...
  /**
   * Number of preconditions of this rule which are still unsatisfied. This number is decreased
   * during the exploration of the relaxed planning graph until it drops to zero and this rule
   * triggers.
   */
