...
public abstract class Operator {
  ...
  /**
   * Denotes if this operator has a non-empty effect, such that it changes the current state.
   */
  ...
  /**
   * Variables which are affected (either as part of an effect or as part of an observation) by this
   * operator.
   */
   ...
   */
  protected Operator(String name, Set<Pair<Integer, Integer>> observation, boolean isAbstracted,
      boolean isCausative, double cost) {
    this.name = name;
  ...
  /**
   * Checks whether this operator is enabled in a given state, i.e., if its preconditions are
   * satisfied.
   *
   * @param state
  ...
  /**
   * Return a copy of this operator. FIXME Shallow or deep copy? (I guess at the moment it is a mix
   * of both. To be save we should create a deep copy).
   *
   * @return
   */
