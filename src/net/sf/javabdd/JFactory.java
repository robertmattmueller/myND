...
public class JFactory extends BDDFactory {
  ...
  private class bdd extends BDD {
     ...
     * (non-Javadoc)
     *
     * @see net.sf.javabdd.BDD#applyAll(net.sf.javabdd.BDD, net.sf.javabdd.BDDFactory.BDDOp,
     * net.sf.javabdd.BDD)
     */
    @Override
     ...
     * (non-Javadoc)
     *
     * @see net.sf.javabdd.BDD#applyEx(net.sf.javabdd.BDD, net.sf.javabdd.BDDFactory.BDDOp,
     * net.sf.javabdd.BDD)
     */
    @Override
     ...
     * (non-Javadoc)
     *
     * @see net.sf.javabdd.BDD#applyUni(net.sf.javabdd.BDD, net.sf.javabdd.BDDFactory.BDDOp,
     * net.sf.javabdd.BDD)
     */
    @Override
     ...
     *
     *         <p>
     *         Finds one random satisfying variable assignment. Finds a minterm in this BDD. The
     *         <tt>var</tt> argument is a set of variables that must be mentioned in the result. The
     *         polarity of these variables in the result - in case they are undefined in this BDD -
     *         are set randomly.
     *
     *         <p>
  ...
  static String errorstrings[] = {"", "Out of memory", "Unknown variable", "Value out of range",
    ...
    "Mismatch in size of variable sets", "Cannot allocate fewer nodes than already in use",
    "Unknown operator", "Illegal variable set", "Bad variable block operation",
    "Trying to decrease the number of variables",
    "Trying to replace with variables already in the bdd",
    "Number of nodes reached user defined maximum", "Unknown BDD - was not in node table",
    "Bad size argument", "Mismatch in bitvector size", "Illegal shift-left/right parameter",
  "Division by zero"};
  ...
  int bdd_support(int r) {
    ...
    /*
     * Update global variables used to speed up bdd_support() - instead of always memsetting support
     * to zero, we use a change counter. - and instead of reading the whole array afterwards, we
  ...
  int satoneset_rec_random(int r, int var) {
    ...
    if (LEVEL(r) < LEVEL(var)) {
      /*
       * Must not occur since BDD r does not contain variables which are not in varset var.
       */
      assert false;
      if (LOW(r) != 0 && HIGH(r) != 0) {
  ...
  {0, 1, 0, 0}, /* less than ( < ) */{1, 0, 1, 1}, /* inverse implication ( << ) */{1, 1, 0, 0} /*
                                                                                                 * not
                                                                                                 * (
                                                                                                 * !
                                                                                                 * )
  ...
  int firstReorder;
  /*
   * Used instead of local variable in order to avoid compiler warning about 'first' being clobbered
   * by setjmp
   */
  ...
  void bdd_checkreorder() {
    ...
    /*
     * And if very little was gained this time (< 20%) then wait until even more nodes (upto twice
     * as many again) have been used
     */
  ...
  void reorder_swapResize(int toBeProcessed, int var0) {
    ...
    while (toBeProcessed != 0) {
      ...
      /*
       * We know that the refcou of the grandchilds of this node is greater than one (these are
       * f00...f11), so there is no need to do a recursive refcou decrease. It is also possible for
       * the node.low/high nodes to come alive again, so deref. of the childs is delayed until the
       * local GBC.
       */
  ...
  int reorder_makenode(int var, int low, int high) {
    ...
    /*
     * Note: We know that low,high has a refcou greater than zero, so there is no need to add
    ...
    if (bddfreepos == 0) {
      ...
      /*
       * Try to allocate more nodes - call noderesize without enabling rehashing. Note: if ever
       * rehashing is allowed here, then remember to update local variable "hash"
       */
  ...
  int reorder_init() {
    ...
    /*
     * First mark and recursive refcou. all roots and childs. Also do some setup here for both
     * setLevellookup and reorder_gbc
     */
    if (mark_roots() < 0) {
      return -1;
    }
  ...
  void dup_level(int levToInsert, int val) {
    for (int n = 2; n < bddnodesize; n++) {
      ...
      bdd_delref(n);
      // System.out.println("Lev = "+lev+" old low = "+lo+" old high = "+hi+" new low = "+n_low+" ("+new
  ...
  static boolean hasEasyFactors(int src) {
    return hasFactor(src, 3) || hasFactor(src, 5) || hasFactor(src, 7) || hasFactor(src, 11)
        || hasFactor(src, 13);
  }
