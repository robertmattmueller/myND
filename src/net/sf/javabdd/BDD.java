 ...
 *
 * <p>
 * Some methods, namely <tt>exist()</tt>, <tt>forall()</tt>, <tt>unique()</tt>, <tt>relprod()</tt>,
 * <tt>applyAll()</tt>, <tt>applyEx()</tt>, <tt>applyUni()</tt>, and <tt>satCount()</tt> take a 'set
 * of variables' argument that is also of type BDD. Those BDDs must be a boolean function that
 * represents the all-true minterm of the BDD variables of interest. They only serve to identify the
 * set of variables of interest, however. For example, for a given BDDDomain, a BDD var set
 * representing all BDD variables of that domain can be obtained by calling <tt>BDDDomain.set()</tt>
 * .
 * </p>
 *
...
public abstract class BDD {
  ...
  public static class BDDIterator implements Iterator {
    ...
    /**
     * <p>
     * Construct a new BDDIterator on the given BDD. The <tt>var</tt> argument is the set of
  ...
  /**
   * <p>
   * This is another version of iterator() that supports the remove() operation. It is much slower
   * than the other one.
   * </p>
   *
   * @return an iteration of minterms
   ...
   * @author Manuela Ortlieb
   */
  protected int printdotfile_rec(FileWriter writer, int current, boolean[] visited,
  ...
  private static void fdd_printset_helper(OutputBuffer sb, BigInteger value, int i, int[] set,
      int[] var, int maxSkip) {
    if (i == maxSkip) {
      // _assert(set[var[i]] == 0);
  ...
  private static void fdd_printset_rec(BDDFactory bdd, StringBuffer sb, BDDToString ts, BDD r,
      int[] set) {
