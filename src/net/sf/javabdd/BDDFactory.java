...
public abstract class BDDFactory {
  ...
  public static BDDFactory init(String bddpackage, int nodenum, int cachesize) {
    ...
    try {
      ...
      return (BDDFactory) m.invoke(null,
          new Object[] {new Integer(nodenum), new Integer(cachesize)});
   ...
   *
   * <p>
   * Note that this operation must walk through the node table many times, and therefore it is much
   * more efficient to call this when the node table is small.
   * </p>
   *
   ...
   *
   * <p>
   * Creates a new variable block with the variables in the variable set var. The variables in var
   * must be contiguous.
   * </p>
   *
   * <p>
   * The fixed parameter sets the block to be fixed (no reordering of its child blocks is allowed)
   * or free.
   * </p>
   *
   * <p>
   ...
   *
   * <p>
   * Adds a variable block for all BDD variables declared so far. Each block contains one variable
   * only. More variable blocks can be added later with the use of addVarBlock -- in this case the
   * tree of variable blocks will have the blocks of single variables as the leafs.
   * </p>
   *
   * <p>
  ...
  public int[] makeVarOrdering(boolean reverseLocal, String ordering) {
    ...
    for (int i = 0;; ++i) {
      ...
      bitIndex =
          fillInVarIndices(doms, i - numberOfDomains, numberOfDomains + 1, localOrders, bitIndex,
              varorder);
      if (!st.hasMoreTokens()) {
        break;
      }
   ...
   */
  static int fillInVarIndices(BDDDomain[] doms, int domainIndex, int numDomains,
